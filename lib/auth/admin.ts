...
const JWT_SECRET = new TextEncoder().encode(
  process.env.ADMIN_JWT_SECRET || process.env.JWT_SECRET || 'aptos-admin-secret-change-in-production'
);
const SALT_LEN = 16;
const KEY_LEN = 64;
const TOKEN_EXP = '7d';
...
export async function requireAdmin(
  ...
  if (!payload) {
    return { payload: null, response: NextResponse.json({ error: 'Invalid or expired token' }, { status: 401 }) };
  }
