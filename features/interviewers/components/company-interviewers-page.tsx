  ...
  const tableItems: InterviewersTableItem[] = useMemo(
    () =>
      interviewers.map((interviewer) => ({
        id: interviewer.id,
        avatar: interviewer.avatarUrl ?? null,
        name: interviewer.name,
        email: interviewer.email,
        phone: interviewer.phone ?? '',
        total_interviews: (interviewer as Interviewer & { totalInterviews?: number }).totalInterviews ?? 0,
        status: 'active'
      })),
    [interviewers]
  );
          ...
          <DialogFooter className="flex justify-end gap-2">
            ...
            <Button
              ...
              onClick={async () => {
                ...
                try {
                  const response = await axios.delete(`/api/interviewers/${interviewerToDelete.id}`);
...
export default CompanyInterviewersPage;
